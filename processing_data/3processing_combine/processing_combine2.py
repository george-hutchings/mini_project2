...
cf_full = np.load('/data/users/uu85g9/confounders2.npy')
...
np.save(save_dir + 'mri_train2.npy', mri_train)
mask_train = np.concatenate(mask_train, axis=0)
np.save(save_dir + 'mask_train2.npy', mask_train)
cf_train = np.concatenate(cf_train, axis=0)
np.save(save_dir + 'cf_train2.npy', cf_train)


mri_val = np.concatenate(mri_val, axis=0)
np.save(save_dir + 'mri_val2.npy', mri_val)
mask_val = np.concatenate(mask_val, axis=0)
np.save(save_dir + 'mask_val2.npy', mask_val)
cf_val = np.concatenate(cf_val, axis=0)
np.save(save_dir + 'cf_val2.npy', cf_val)
